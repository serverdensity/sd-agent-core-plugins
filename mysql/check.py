...
class MySql(AgentCheck):
    ...
    def _collect_metrics(self, host, db, tags, options, queries):
        ...
        if _is_affirmative(options.get('extra_performance_metrics', False)) and above_560 and \
            ...
            # report avg query response time per schema to Server Density
        ...
        if _is_affirmative(options.get('replication', False)):
            ...
            if self._version_compatible(db, host, (5, 7, 0)):
                ...
                if slave_io_running:
                    slave_io_running = any(v.lower().strip() == 'yes' for v in slave_io_running.values())
                if slave_sql_running:
                    slave_sql_running = any(v.lower().strip() == 'yes' for v in slave_sql_running.values())
        ...
        try:
            with closing(db.cursor()) as cursor:
                ...
                if result is not None:
                    for field in list(field_metric_map.keys()):
    ...
    def _get_binary_log_stats(self, db):
        try:
            with closing(db.cursor()) as cursor:
                ...
                binary_log_space = 0
                for key, value in master_logs.items():
    ...
    def _get_stats_from_innodb_status(self, db):
        ...
        for line in innodb_status_text.splitlines():
            ...
            if line.find('Mutex spin waits') == 0:
                ...
                results[
                    'Innodb_semaphore_wait_time'] += int(float(row[9])) * 1000
            ...
            # TRANSACTIONS
                ...
                results['Innodb_row_lock_time'] += int(row[5]) * 1000
                ...
                try:
                    if len(row) == 8:
                        ...
                        if self._are_values_numeric(row[4:8]) and self._are_values_numeric(row[11:15]):
                            results['Innodb_pending_normal_aio_reads'] = (int(row[4]) + int(row[5]) +
                                                                      int(row[6]) + int(row[7]))
                            results['Innodb_pending_normal_aio_writes'] = (int(row[11]) + int(row[12]) +
            ...
            # INSERT BUFFER AND ADAPTIVE HASH INDEX
                ...
                results['Innodb_hash_index_cells_used'] = int(
