...
VM_MONITORING_FLAG = 'SDMonitored'
...
class VSphereEvent(object):
    ...
    def get_serverdensity_payload(self):
...
class VSphereCheck(AgentCheck):
    """ Get performance metrics from a vCenter server and upload them to Server Density
    References:
    ...
    *_atomic jobs perform one single task asynchronously in the ThreadPool, we
    ...
    def _query_event(self, instance):
        ...
        try:
            ...
            for event in new_events:
                ...
                event_payload = normalized_event.get_serverdensity_payload()
    ...
    def _cache_morlist_process_atomic(self, instance, mor):
        ...
        ### <TEST-INSTRUMENTATION>
        self.histogram('serverdensity.agent.vsphere.morlist_process_atomic.time', t.total())
        ### </TEST-INSTRUMENTATION>
    ...
    def _cache_metrics_metadata(self, instance):
        ...
        ### <TEST-INSTRUMENTATION>
        self.histogram('serverdensity.agent.vsphere.metric_metadata_collection.time', t.total())
        ### </TEST-INSTRUMENTATION>
    ...
    def _collect_metrics_atomic(self, instance, mor):
        ...
        ### <TEST-INSTRUMENTATION>
        self.histogram('serverdensity.agent.vsphere.metric_colection.time', t.total())
        ### </TEST-INSTRUMENTATION>
    ...
    def check(self, instance):
        if not self.pool_started:
            self.start_pool()
        ### <TEST-INSTRUMENTATION>
        self.gauge('serverdensity.agent.vsphere.queue_size', self.pool._workq.qsize(), tags=['instant:initial'])
        ### </TEST-INSTRUMENTATION>
        ...
        ### <TEST-INSTRUMENTATION>
        self.gauge('serverdensity.agent.vsphere.queue_size', self.pool._workq.qsize(), tags=['instant:final'])
        ### </TEST-INSTRUMENTATION>
